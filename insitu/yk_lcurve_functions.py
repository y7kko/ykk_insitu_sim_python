...
Bruno edit:
- Imposição de valor mínimo de lambda ao chute inicial da curvatura.
Essa alteração impede overfitting à dados experimentais
"""
...
def l_corner(rho,eta,reg_param,u,sig,bm):
    ...
    thresh = np.where(reg_param>=_reguparam_thresh)[0]
    curv_id = np.argmin(curv[thresh]+thresh[-1])
...
def set_module_options(**kwargs):
    if 'minlambda' in kwargs:
        global _reguparam_thresh
        reguparam_thresh = kwargs.pop('minlambda')
        _reguparam_thresh = reguparam_thresh
